...
APP_NAME="Prompt Output Grader"
BUNDLE_ID="com.mototax.promptoutputgrader"
...
DEFAULTS_SOURCE_DIR="${SCRIPT_DIR}/Defaults"
...
echo "Building bundled grader..."
rm -rf "${RUNNER_DIST}" "${RUNNER_WORK}" "${RUNNER_SPEC}"
"${VENV_DIR}/bin/python" -m PyInstaller \
  --onefile \
  --clean \
  --name prompt_output_grader \
  --distpath "${RUNNER_DIST}" \
  --workpath "${RUNNER_WORK}" \
  --specpath "${RUNNER_SPEC}" \
  "${REPO_ROOT}/script/prompt_output_grader.py"

echo "Building Swift app..."
swift build -c release --package-path "${SCRIPT_DIR}" >/dev/null
SWIFT_BIN="$(swift build -c release --package-path "${SCRIPT_DIR}" --show-bin-path)/PromptGradeApp"
...
draw.rounded_rectangle((244, 286, 780, 378), radius=46, fill=(16, 133, 88, 212))
for x in (352, 462, 572, 682):
    draw.line((x, 418, x, 702), fill=(48, 84, 70, 54), width=5)
for y in (438, 512, 586, 660):
    draw.line((244, y, 780, y), fill=(48, 84, 70, 54), width=5)

for y, colors in [
    (456, ((45, 212, 122), (255, 255, 255))),
    (530, ((255, 190, 86), (255, 255, 255))),
    (604, ((34, 197, 94), (255, 255, 255))),
]:
    draw.rounded_rectangle((274, y, 320, y + 24), radius=12, fill=colors[0] + (218,))
    draw.rounded_rectangle((352, y + 2, 646, y + 18), radius=9, fill=(42, 75, 62, 80))
    draw.rounded_rectangle((676, y + 2, 748, y + 18), radius=9, fill=(42, 75, 62, 54))
...
check = Image.new("RGBA", icon.size, (0, 0, 0, 0))
draw = ImageDraw.Draw(check)
draw.line((672, 706, 720, 752, 790, 660), fill=(255, 255, 255, 248), width=26, joint="curve")
draw.arc((632, 610, 818, 796), 210, 330, fill=(255, 255, 255, 92), width=7)
draw.ellipse((678, 640, 716, 678), fill=(255, 255, 255, 64))
icon.alpha_composite(check)
...
rm -rf "${APP_DIR}"
mkdir -p "${APP_DIR}/Contents/MacOS" "${APP_DIR}/Contents/Resources/Defaults"
cp "${SWIFT_BIN}" "${APP_DIR}/Contents/MacOS/PromptGradeApp"
cp "${RUNNER_DIST}/prompt_output_grader" "${APP_DIR}/Contents/Resources/prompt_output_grader"
cp "${ICON_PATH}" "${APP_DIR}/Contents/Resources/AppIcon.icns"
if [[ -d "${DEFAULTS_SOURCE_DIR}" ]]; then
  ditto "${DEFAULTS_SOURCE_DIR}" "${APP_DIR}/Contents/Resources/Defaults"
...
plist = {
    ...
    "CFBundleExecutable": "PromptGradeApp",
...
echo "Bundled grader: ${APP_DIR}/Contents/Resources/prompt_output_grader"
